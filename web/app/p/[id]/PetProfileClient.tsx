...
import { getPetCtaPlan, ctaLabel, isForSale, LOCKED_SECTION_COPY, type Locale, type PetCtaKind } from '@/lib/publicProfile/statusLabels';
import VerifiedBadge from '@/app/components/public/VerifiedBadge';
import StatusPill from '@/app/components/public/StatusPill';
import PublicPetCard, { type PublicPetCardData } from '@/app/components/public/PublicPetCard';
import ContactModal from '@/app/components/public/ContactModal';
...
export default function PublicPetProfilePage() {
  ...
  const [otherPets, setOtherPets] = useState<PublicPetCardData[]>([]);
  ...
  const fetchPublicData = async () => {
    try {
      ...
      // ตัด `weight` (คอลัมน์เก่าที่เลิกใช้แล้ว แหล่งจริงคือ pet_weights) และ `note`
      // (ฟิลด์บันทึกภายในของฟาร์ม ไม่เคยแสดงผลที่ไหนเลย) ออกจาก select สาธารณะนี้ด้วย —
      // ทั้งคู่เป็น dead weight ที่หลุดไปถึงเบราว์เซอร์ของผู้ชมนิรนามโดยไม่มีประโยชน์ใด ๆ
      const { data: petData, error: petError } = await supabase
        ...
        .select(`
          ...
          species, farm_id, price, microchip_number,
      ...
      // สัตว์นี้เป็นของฟาร์ม (ไม่ใช่ 'PERSONAL') ไหม — ตัดสินใจครั้งเดียว ใช้ประกอบเงื่อนไข query ด้านล่าง
      const petIsFarmPet = !!(petData.farm_id && petData.farm_id !== 'PERSONAL');

      // ─── query ที่เป็นอิสระต่อกันทั้งหมด (ไม่ต้องรอผลกัน) ยิงพร้อมกันด้วย Promise.all ───
      const [farmRes, otherPetsRes, ownerRes, vaccineRes, activityRes, weightRes] = await Promise.all([
        petIsFarmPet
          ? supabase.from('farms').select('id, farm_name, phone, facebook_link, line_id, is_verified').eq('id', petData.farm_id).maybeSingle()
          : Promise.resolve({ data: null }),
        // สัตว์อื่นจากฟาร์มเดียวกัน (เฉพาะที่เปิดเผยสาธารณะ ไม่รวมตัวปัจจุบัน) — จำกัด 4 ตัว
        petIsFarmPet
          ? supabase.from('pets').select('id, name, breed, image_url, gender, status, price, birth_date').eq('farm_id', petData.farm_id).eq('is_public', true).neq('id', petData.id).limit(4)
          : Promise.resolve({ data: null }),
        // เจ้าของ (แสดงชื่อ + ฟาร์ม แต่ไม่โชว์เบอร์/อีเมลส่วนตัวในมุมมองสาธารณะ)
        petData.user_id
          ? supabase.from('profiles').select('id, full_name, avatar_url, address').eq('id', petData.user_id).maybeSingle()
          : Promise.resolve({ data: null }),
      ...
      if (otherPetsRes.data) setOtherPets(otherPetsRes.data as PublicPetCardData[]);
      if (ownerRes.data) setOwner(ownerRes.data as UserProfile);
  ...
  // ─── ติดต่อฟาร์ม: log lead — ปุ่มเปิดลิงก์/ปิด modal อยู่ใน ContactModal เอง ───
  const logLead = async (channel: "phone" | "line" | "facebook") => {
    try {
      const { data: { session } } = await supabase.auth.getSession();
                      ...
                      <div className="ped-card-link">
                        <div className={`ped-card ${isSelfCol ? 'ped-card-self' : ''} ${node.isMissing ? 'ped-card-missing' : ''} ${isSireSide ? 'ped-sire' : 'ped-dam'}`}>
                          <div className="ped-card-img">
                            {node.image_url ? <img src={node.image_url} alt={node.name || ''} loading="lazy" /> : (node.isMissing ? <img src="/icons/icon-paw-circle-white.png" alt="" style={{ width: '60%', height: '60%', objectFit: 'contain' }} /> : (isSireSide ? <Icon.Male /> : <Icon.Female />))}
                          </div>
  ...
  const isOwnPet = !!sessionUserId && sessionUserId === pet.user_id;
  const shareUrl = typeof window !== 'undefined' ? window.location.href : '';
  const ctaPlan = getPetCtaPlan({
    status: pet.status,
    isFarmPet: !!isFarmPet,
    isLoggedIn: !!sessionUserId,
    isOwnPet,
    hasContact,
    myReservationStatus: myReservation ? (myReservation.status as 'pending' | 'confirmed') : null,
  });
  const hiddenTabCount = fieldAccess ? TABS.filter(tab => fieldAccess[tab.fieldGroupKey] === false).length : 0;
  ...
  // ─── Renders one CTA slot per the shared getPetCtaPlan rules ───
  const renderPetCta = (kind: PetCtaKind, key: string) => {
    switch (kind) {
      case 'reserve':
        return (
          <button key={key} className="contact-btn" style={{ background: '#16A34A' }} onClick={handleReserve} disabled={reserving}>
            {reserving ? 'กำลังจอง...' : `📌 ${ctaLabel('reserve', locale)}`}
          </button>
        );
      case 'login_to_reserve':
        return (
          <a key={key} href={`/login?redirect=${encodeURIComponent(`/p/${petId}`)}`} className="status-pill" style={{ background: '#EFF6FF', color: '#2563EB', border: '1px solid #BFDBFE', textDecoration: 'none' }}>
            {ctaLabel('login_to_reserve', locale)}
          </a>
        );
      case 'reserved_pending':
        return (
          <span key={key} className="status-pill" style={{ background: '#FEF3C7', color: '#92400E', border: '1px solid #FDE68A' }}>
            ⏳ {ctaLabel('reserved_pending', locale)}
          </span>
        );
      case 'reserved_confirmed':
        return (
          <span key={key} className="status-pill" style={{ background: '#D1FAE5', color: '#065F46', border: '1px solid #A7F3D0' }}>
            ✓ {ctaLabel('reserved_confirmed', locale)}
          </span>
        );
      case 'contact':
        return (
          <button key={key} className="contact-btn" onClick={() => setShowContactModal(true)}>
            <Icon.Message /> {ctaLabel('contact', locale)}
          </button>
        );
      case 'view_farm':
        return farm ? (
          <Link key={key} href={`/farm/${farm.id}`} className="status-pill" style={{ background: '#F3F4F6', color: F.inkSoft, border: `1px solid ${F.lineMid}`, textDecoration: 'none' }}>
            <Icon.Home /> {ctaLabel('view_farm', locale)}
          </Link>
        ) : null;
      case 'share':
        return (
          <button key={key} className="contact-btn" style={{ background: 'white', color: F.pink, border: `1px solid ${F.pinkBorder}` }} onClick={handleShare}>
            {copied ? (locale === 'th' ? 'คัดลอกลิงก์แล้ว' : 'Link copied') : ctaLabel('share', locale)}
          </button>
        );
      default:
        return null;
  ...
  const handleShare = () => {
    share({
      title: pet.name || 'Whiskora',
      text: locale === 'th'
        ? `ดูโปรไฟล์ของ ${pet.name} จาก ${farm?.farm_name || 'Whiskora'} บน Whiskora`
        : `See ${pet.name}'s profile from ${farm?.farm_name || 'Whiskora'} on Whiskora`,
      url: shareUrl,
    });
  };
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .hero-main-image { position: relative; flex-shrink: 0; width: 280px; height: 280px; border-radius: 20px; overflow: hidden; border: 1px solid ${F.pinkBorder}; box-shadow: 0 4px 24px rgba(232,70,119,.08); cursor: zoom-in; }
        ...
        <div className="page-body">
          ...
          <div className="hero-section">
            ...
            <div className="hero-main-image" onClick={() => setLightboxImage(selectedImage || pet.image_url || null)}>
              <Image src={selectedImage || pet.image_url || '/placeholder-pet.jpg'} alt={pet.name} fill sizes="(max-width: 768px) 100vw, 280px" style={{ objectFit: 'cover' }} priority />
            </div>
            <div className="hero-info">
              {farm?.is_verified && <div style={{ marginBottom: 6 }}><VerifiedBadge locale={locale} /></div>}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                {pet.status && <StatusPill status={pet.status} isFarmPet={!!isFarmPet} locale={locale} />}
                {isForSale(pet.status, !!isFarmPet) && pet.price != null && Number(pet.price) > 0 && (
                  <span className="price-pill"><Icon.Tag /> ฿{Number(pet.price).toLocaleString()}</span>
                )}
                {renderPetCta(ctaPlan.primary, 'primary')}
                {ctaPlan.secondary !== ctaPlan.primary && ctaPlan.secondary !== 'none' && renderPetCta(ctaPlan.secondary, 'secondary')}
              </div>
            </div>
          </div>
          ...
          {hiddenTabCount > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: F.muted, margin: '-16px 0 20px', padding: '8px 12px', background: '#FAFAFA', borderRadius: 10 }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><circle cx="12" cy="12" r="10" /><line x1="12" y1="16" x2="12" y2="12" /><line x1="12" y1="8" x2="12.01" y2="8" /></svg>
              {sessionUserId ? LOCKED_SECTION_COPY.insufficient_tier[locale] : LOCKED_SECTION_COPY.anonymous[locale]}
            </div>
          )}
          ...
          {activeTab === 'overview' && (
            <div className="content-grid">
              <div className="content-main">
                {healthDetails?.traits && (
                  <div className="card">
                    <div className="card-header"><div className="card-title"><div className="card-title-icon" style={{ background: F.pinkSoft }}><img src="/icons/icon-paw-circle-white.png" alt="" style={{ width: 18, height: 18, objectFit: 'contain' }} /></div>เกี่ยวกับน้อง</div></div>
                    <div className="card-body">
                      <p style={{ fontSize: 13, color: F.inkSoft, lineHeight: 1.7, margin: 0 }}>{healthDetails.traits}</p>
                    </div>
                  </div>
                )}
                  ...
                  <div className="card">
                    ...
                    <div className="card-body">
                      <div className="gallery-grid">
                        {allImages.map((img, i) => (
                          <div key={i} className="gallery-item"><img src={img} alt="" loading="lazy" onClick={() => setLightboxImage(img)} /></div>
                        ))}
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="timeline-list">
                        {combinedTimeline.slice(0, 5).map(item => (
                          <div key={item.id} className="timeline-item"><div className="timeline-dot" style={{ background: item.color }} />
                            ...
                            {item.image_url && <div className="timeline-img"><img src={item.image_url} alt="" loading="lazy" /></div>}
                          </div>
                        ))}
                      </div>
                    )}
              ...
              <div className="content-main">
                <div className="card">
                  ...
                  <div className="card-body">
                    ...
                    {healthDetails?.health_notes && (
                      <div style={{ marginTop: '12px', background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: '10px', padding: '14px' }}>
                        <div style={{ fontSize: '10px', fontWeight: 700, color: '#D97706', textTransform: 'uppercase', marginBottom: '6px' }}>บันทึกสุขภาพเพิ่มเติม</div>
                        <p style={{ fontSize: '13px', color: '#92400E', fontWeight: 600, lineHeight: 1.6 }}>{healthDetails.health_notes}</p>
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="timeline-list">
                    {combinedTimeline.map(item => (
                      <div key={item.id} className="timeline-item"><div className="timeline-dot" style={{ background: item.color }} />
                        ...
                        <div className="timeline-title">{item.title}{item.tag && <span className="timeline-id-badge">{item.tag}</span>}</div>
                        {item.description && <div className="timeline-desc">{item.description}</div>}
                        {item.image_url && <div className="timeline-img"><img src={item.image_url} alt="" loading="lazy" /></div>}</div>
                    ))}
                  </div>
                )}
              </div>
          ...
          {/* ─── สัตว์อื่นจากฟาร์มเดียวกัน ─── */}
          {isFarmPet && otherPets.length > 0 && (
            <div className="card" style={{ marginTop: 24 }}>
              <div className="card-header">
                <div className="card-title"><div className="card-title-icon" style={{ background: F.pinkSoft }}><Icon.Paw /></div>สัตว์อื่นจากฟาร์มเดียวกัน</div>
                {farm && <Link href={`/farm/${farm.id}`} style={{ fontSize: 11, color: F.pink, fontWeight: 600, textDecoration: 'none', flexShrink: 0 }}>ดูสัตว์ทั้งหมดจากฟาร์มนี้ →</Link>}
              </div>
              <div className="card-body">
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 12 }}>
                  {otherPets.map(op => <PublicPetCard key={op.id} pet={op} isFarmPet locale={locale} />)}
                </div>
              </div>
            </div>
          )}

          {/* ─── จองยังไง — เฉพาะเมื่อสถานะยังเปิดจองจริง ─── */}
          {isForSale(pet.status, !!isFarmPet) && (
            <div className="card" style={{ marginTop: 24 }}>
              <div className="card-header"><div className="card-title"><div className="card-title-icon" style={{ background: '#EFF6FF', color: '#2563EB' }}>
              ...
              </div>จองยังไง</div></div>
              <div className="card-body">
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 16 }}>
                  {[
                    { n: 1, th: 'ส่งคำขอจองผ่านหน้านี้ (ต้องเข้าสู่ระบบ)', en: 'Send a reservation request from this page (login required)' },
                    { n: 2, th: 'ฟาร์มตรวจสอบและยืนยันคำขอ', en: 'The farm reviews and confirms your request' },
                    { n: 3, th: 'นัดหมายรับน้องกับฟาร์มโดยตรง', en: 'Coordinate pickup directly with the farm' },
                  ].map(step => (
                    <div key={step.n} style={{ textAlign: 'center' }}>
                      <div style={{ width: 28, height: 28, borderRadius: '50%', background: F.pinkSoft, color: F.pink, fontWeight: 700, fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 8px' }}>{step.n}</div>
                      <div style={{ fontSize: 12, color: F.inkSoft, lineHeight: 1.5 }}>{locale === 'th' ? step.th : step.en}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="owner-footer">
              <div className="owner-footer-main">
                ...
                <div className="owner-footer-info">
                  ...
                  {farm?.is_verified && <div className="owner-cattery"><VerifiedBadge locale={locale} size="sm" /></div>}
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button className="contact-btn" style={{ background: 'white', color: F.pink, border: `1px solid ${F.pinkBorder}` }} onClick={handleShare}>
                  {copied ? (locale === 'th' ? 'คัดลอกลิงก์แล้ว' : 'Link copied') : ctaLabel('share', locale)}
                </button>
              </div>
            </div>
      ...
      <ContactModal
        ...
        title={`ติดต่อ${isFarmPet ? `ฟาร์ม ${farm?.farm_name || ''}` : 'เจ้าของ'}`}
        subtitle="เลือกช่องทางที่สะดวก แล้วทักได้เลย"
        phone={farm?.phone}
        lineId={farm?.line_id}
        facebookLink={farm?.facebook_link}
        onChannelClick={(channel) => logLead(channel)}
      />
