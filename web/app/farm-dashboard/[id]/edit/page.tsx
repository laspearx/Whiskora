...
export default function EditFarmPage() {
  ...
  const handleUploadAvatar = async () => {
    ...
    try {
      ...
      setImageUrl(await uploadToStorage(blob, `farms/${session.user.id}/${farmId}-logo-${Date.now()}.jpg`));
      setAvatarSrc(null);
    } catch { alert("อัปโหลดรูปโปรไฟล์ไม่สำเร็จ"); } finally { setUploadingAvatar(false); }
  };
  ...
  const handleUploadCover = async () => {
    ...
    try {
      ...
      setCoverUrl(await uploadToStorage(blob, `farms/${session.user.id}/${farmId}-cover-${Date.now()}.jpg`));
      setCoverSrc(null);
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .fe-avatar-circle { width: 110px; height: 110px; border-radius: 50%; overflow: hidden; background: ${F.pinkSoft}; border: 3px solid white; box-shadow: 0 4px 16px rgba(232,70,119,0.15); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: box-shadow .18s; }
      ...
      <div className="fe-page">
        <div className="fe-body">
              ...
              {coverUrl ? (
                <>
                  ...
                  <div className="fe-cover-actions">
                    <button type="button" className="fe-cover-btn fe-cover-btn-edit" onClick={e => { e.stopPropagation(); coverInputRef.current?.click(); }}><Icon.Camera /> เปลี่ยน</button>
                    <button type="button" className="fe-cover-btn fe-cover-btn-del" onClick={e => { e.stopPropagation(); setCoverUrl(null); }}><Icon.Trash /> ลบ</button>
                  </div>
                </>
              ) : (
          ...
          <div className="fe-avatar-section">
            ...
            {imageUrl && <button type="button" className="fe-avatar-del" onClick={() => setImageUrl(null)}><Icon.Trash /> ลบรูปโปรไฟล์</button>}
          </div>
        ...
        {/* Members link */}
        <div style={{ maxWidth: 600, margin: "0 auto", padding: "0 20px 8px" }}>
          <a
          ...
          <div className="fe-modal-card">
            ...
            <div className="fe-modal-body">
              ...
              <div className="fe-modal-btns">
                ...
                <button className="fe-btn fe-btn-save" onClick={handleUploadAvatar} disabled={uploadingAvatar}>{uploadingAvatar ? "กำลังอัปโหลด..." : "ยืนยัน"}</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="fe-modal-card" style={{ maxWidth: 480 }}>
            ...
            <div className="fe-modal-body">
              ...
              <div className="fe-modal-btns">
                ...
                <button className="fe-btn fe-btn-save" onClick={handleUploadCover} disabled={uploadingCover}>{uploadingCover ? "กำลังอัปโหลด..." : "ยืนยัน"}</button>
              </div>
            </div>
          </div>
        </div>
