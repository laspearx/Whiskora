...
export default function AdminShopsPage() {
  ...
  const fetchShops = useCallback(async (q: string, p: number) => {
    setFetching(true);
    let query = supabase
      .from('shops')
      .select('id, shop_name, image_url, owner_name, user_id, created_at', { count: 'exact' })
      .order('created_at', { ascending: false })
      .range(p * PAGE_SIZE, p * PAGE_SIZE + PAGE_SIZE - 1);

    const safeQ = sanitizeOrTerm(q);
    if (safeQ) query = query.or(`shop_name.ilike.%${safeQ}%,owner_name.ilike.%${safeQ}%`);
    ...
    if (!error && data) {
      ...
      const rows: ShopRow[] = data.map((s: any) => {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      ...
      setPage(0);
      if (!loading) fetchShops(searchInput, 0);
    }, 350);
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .ad-shop-card { display: flex; align-items: center; gap: 14px; background: white; border: 1px solid ${F.lineMid}; border-radius: 14px; padding: 12px 16px; margin-bottom: 8px; }
        .ad-shop-avatar { width: 52px; height: 52px; border-radius: 50%; object-fit: cover; background: ${F.line}; flex-shrink: 0; display: flex; align-items: center; justify-content: center; }
        .ad-shop-avatar img { width: 100%; height: 100%; object-fit: cover; border-radius: 50%; }
        .ad-shop-avatar-placeholder { width: 55%; height: 55%; opacity: .3; object-fit: contain; }
        .ad-shop-info { flex: 1; min-width: 0; }
        .ad-shop-name { font-size: 14px; font-weight: 700; color: ${F.ink}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .ad-shop-owner { font-size: 12px; color: ${F.muted}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      ...
      <div className="ad-page">
        <div className="ad-body">
          ...
          <div className="ad-top">
            ...
            <div>
              <div className="ad-title">ร้านค้าทั้งหมด</div>
              <div className="ad-sub">รายชื่อเพ็ทช็อปพาร์ทเนอร์ใน Whiskora</div>
            </div>
          </div>
          ...
          <div style={{ marginBottom: 14 }}>
            <span className="ad-count-chip">{totalCount.toLocaleString()} ร้าน</span>
          </div>

          {fetching ? (
            <div className="ad-empty">กำลังค้นหา...</div>
          ) : shops.length === 0 ? (
            <div className="ad-empty">ไม่พบร้านค้าที่ตรงกับคำค้นหา</div>
          ) : (
            shops.map(s => (
              <div key={s.id} className="ad-shop-card">
                <div className="ad-shop-avatar">
                  {s.image_url
                    ? <img src={s.image_url} alt={s.shop_name || ''} />
                    : <img className="ad-shop-avatar-placeholder" src="/icons/icon-shop.png" alt="" />
                  }
                </div>
                <div className="ad-shop-info">
                  <div className="ad-shop-name">{s.shop_name || 'ไม่ระบุชื่อร้าน'}</div>
                  <div className="ad-shop-owner">เจ้าของ: {s.owner_name || 'ไม่ระบุ'}</div>
                </div>
              </div>
            ))
          )}
