...
import { isForSale, type Locale } from '@/lib/publicProfile/statusLabels';
...
export default function PublicFarmProfile() {
  ...
  useEffect(() => {
    const fetchFarmProfile = async () => {
      try {
        // เลือกเฉพาะคอลัมน์ที่หน้านี้แสดงผลจริง — ไม่ select('*') เพราะ farms มี
        // verification_note/verification_status (ฟิลด์ตรวจสอบภายใน ไม่เคยแสดงผลที่ไหนเลย)
        // และที่อยู่แบบละเอียด (house_no/room_no/moo/soi/road/sub_district/postal_code)
        // ที่หน้านี้ไม่ได้ใช้ (ใช้แค่ address/lat/lng สำหรับลิงก์แผนที่) — ทั้งหมดนี้เป็นข้อมูล
        // ที่ไม่ควรหลุดไปถึงผู้ชมนิรนามโดยไม่มีเหตุผล
        const { data: farmData, error: farmError } = await supabase
          .from('farms')
          .select('id, user_id, farm_name, species, phone, bio, image_url, facebook_link, line_id, is_verified, verified_at, cover_url, owner_name, address, lat, lng, province, created_at')
          .eq('id', farmId)
          .single();
        ...
        await Promise.all([
          ...
          farmData.user_id
            ? supabase.from('profiles').select('full_name, username, email, address').eq('id', farmData.user_id).maybeSingle()
              .then(({ data: ownerData }) => { if (ownerData) setOwner(ownerData); })
            : Promise.resolve(),

          // กรองที่ query ตรง ๆ (is_public=true) แทนการดึงทุกตัวมาแล้วค่อยกรองฝั่ง client —
          // ลดโอกาสที่สัตว์ที่ฟาร์มไม่ได้ตั้งใจเปิดเผยจะหลุดมาอยู่ใน state โดยไม่จำเป็น
          supabase.from('pets').select('id, name, breed, image_url, gender, status, price, birth_date')
  ...
  const readyPets = pets.filter(p => isForSale(p.status, true));
  const breederPets = pets.filter(p => p.status === PET_STATUS.BREEDER);
  ...
  // ─── บันทึก contact lead — เดิมมีแค่ /p/[id] ที่ log ได้ ตอนนี้หน้าฟาร์มก็ log ด้วย ───
  const logLead = async (channel: ContactChannel) => {
    try {
      const { data: { session } } = await supabase.auth.getSession();
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        /* ── Sticky bottom CTA ── */
        .fp-cta-bar { position: fixed; bottom: 0; left: 0; right: 0; z-index: 60; background: rgba(255,255,255,0.95); backdrop-filter: blur(10px); border-top: 1px solid ${F.lineMid}; padding: 14px 20px calc(14px + env(safe-area-inset-bottom, 0px)); }
        .fp-trust-row { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 14px; }
        .fp-trust-chip { display: inline-flex; align-items: center; gap: 5px; padding: 5px 12px; border-radius: 14px; font-size: 11px; font-weight: 600; background: white; color: ${F.inkSoft}; border: 1px solid ${F.lineMid}; }
      ...
      <div className="fp-page">
        <div className="fp-body">
          ...
          <div className="fp-cover">
            {farm.cover_url || farm.image_url ? <Image src={farm.cover_url || farm.image_url} alt={farm.farm_name} fill sizes="100vw" style={{ objectFit: 'cover' }} priority /> : null}
          ...
          <div className="fp-identity">
            ...
            {/* ── Trust summary — เฉพาะสัญญาณจริงที่มาจากข้อมูลที่ดึงมาแล้ว ไม่มีคะแนนสมมติ ── */}
            <div className="fp-trust-row">
              <span className="fp-trust-chip">🐾 {pets.length} ตัวในระบบ</span>
              {readyPets.length > 0 && <span className="fp-trust-chip">📌 เปิดจอง/พร้อมย้าย {readyPets.length} ตัว</span>}
              {breederPets.length > 0 && <span className="fp-trust-chip">🧬 มีข้อมูลพ่อพันธุ์แม่พันธุ์</span>}
            </div>
          </div>
          ...
          <div className="fp-section">
            <div className="fp-section-card">
              ...
              <div className="fp-info-grid">
                ...
                {farm.phone && safeTelHref(farm.phone) && (
                  <a className="fp-info-row fp-info-row-link" href={safeTelHref(farm.phone)!}>
                  ...
                  {readyPets.map(pet => (
                    <Link key={pet.id} href={`/p/${pet.id}`} className="fp-pet-card">
                      <div className="fp-pet-img">
                        {pet.image_url ? <img src={pet.image_url} alt={pet.name} loading="lazy" /> : <img src={isMale(pet.gender) ? '/icons/icon-men.png' : '/icons/icon-women.png'} alt="" loading="lazy" style={{width:'60%',height:'60%',objectFit:'contain',opacity:0.4}} />}
          ...
          {/* ── พ่อพันธุ์ / แม่พันธุ์ — แยกจากสัตว์พร้อมย้ายบ้านชัดเจน ไม่ให้เข้าใจผิดว่าขายทั้งหมด ── */}
          {breederPets.length > 0 && (
            <div className="fp-section">
              <div className="fp-section-card">
                <div className="fp-section-head">
                  <div className="fp-section-title"><img src="/icons/icon-breeding.png" alt="" style={{ width: 34, height: 34, objectFit: 'contain' }} /> พ่อพันธุ์ / แม่พันธุ์ในความดูแลของฟาร์ม</div>
                </div>
                <div className="fp-pets-grid">
                  {breederPets.map(pet => (
                    <PublicPetCard key={pet.id} pet={pet as PublicPetCardData} isFarmPet locale={locale} />
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="fp-cta-bar">
            <div className="fp-cta-inner">
              ...
              {(farm.phone || farm.line_id || farm.facebook_link) && (
                <button className="fp-cta-btn fp-cta-primary" onClick={() => setShowContactModal(true)}>
                  <img src="/icons/icon-phone.png" alt="" style={{ width: 18, height: 18, objectFit: 'contain', filter: 'brightness(0) invert(1)' }} /> ติดต่อฟาร์ม
                </button>
              )}
            </div>
          </div>
        ...
        <ContactModal
          ...
          title={`ติดต่อฟาร์ม ${farm.farm_name || ''}`}
